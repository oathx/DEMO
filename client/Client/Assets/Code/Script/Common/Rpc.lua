local hashString = GUtility.HashString
...
local function _register(rpc, protocol, owner, func)
    local lf = ProtocolImp[protocol]
    if not lf then
        ProtocolImp[protocol] = function() end
    end
...
local function _parse_tcp_server(rpc)
    ...
    if prop then
        local InternalPacketType = {
            [PacketType.SOCKET_CONNECT_SUCCESS] = ProtocolImp.NetConnectSuccess,
            [PacketType.SOCKET_CONNECT_FAILURE] = ProtocolImp.NetConnectFailure,
            [PacketType.SOCKET_DISCONNECT]      = ProtocolImp.NetDisconnect
        }
