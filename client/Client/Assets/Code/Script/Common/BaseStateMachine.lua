module("BaseStateMachine", package.seeall)

function new()
    local machine = {
    }
    setmetatable(machine, {__index=BaseStateMachine})
    machine:Init()
...
function AddState(self, name, fnEnter, fnExit, fnEvent)
    local state = {
    }
...
function FireEvent(self, id, args)
    local state = self:GetCurrentState()
    if state then
        if state.fnEvent then
            state.fnEvent(self, id, args)
        end
    end
end
