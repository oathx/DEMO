...
public class Lua_XResourceManager : LuaObject {
	[SLua.MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	[UnityEngine.Scripting.Preserve]
	static public int LoadAsync(IntPtr l) {
		try {
			XResourceManager self=(XResourceManager)checkSelf(l);
			System.String a1;
	...
	static public int GetSingleton_s(IntPtr l) {
		try {
			var ret=XResourceManager.GetSingleton();
			pushValue(l,true);
			pushValue(l,ret);
			return 2;
		}
	...
	static public void reg(IntPtr l) {
		getTypeTable(l,"XResourceManager");
		addMember(l,LoadAsync);
		addMember(l,LoadMultiAsync);
		addMember(l,LoadScene);
		addMember(l,GetSingleton_s);
		createTypeMetatable(l,null, typeof(XResourceManager),typeof(UnityEngine.MonoBehaviour));
	}
}
