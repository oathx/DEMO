...
setmetatable(UIWidget, {__index=LuaBehaviour})
...
function RegisterClickEvent(self, szName, evtCallback)
    ...
    INFO("register clicked event [%s](%s)",
        szName, evtCallback)

	UIEventTriggerListener.Get(widget, self).onClick = evtCallback
end

function RegisterClickObject(self, target, evtCallback)
	UIEventTriggerListener.Get(target, self).onClick = evtCallback
end
	...
	for idx, name in ipairs(aryMove) do
		...
		if rectTrans then
			...
			if flag then
				...
				newPos = Vector3(curPos.x + rectTrans.rect.width, curPos.y, curPos.z)
				twEase = XEase.InBack
				fSpeed = 0.2				
			end
