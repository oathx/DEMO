...
fixed3 _EBGAfterImageTint;
float _EBGAfterImageRamp;

inline fixed EBGFogVertex(float4 vert)
{
	float4 worldPos = mul(unity_ObjectToWorld, vert); 
	half dist = length(worldPos.xyz - _WorldSpaceCameraPos); 
...
inline fixed3 EBGPointLightWordPos(half3 worldPos, float3 worldN)
{
	...
	fixed4 atten = (fixed4)clamp((diff * _EBGPointLightIntensity) / (lengthSq * _EBGPointLightMultiplier + 1.0), -2, 2);
